...
Each phylogenetic character is a statement about a particular variable phenotype observed in one or more organisms.
...
In such a format, they cannot be easily parsed or reasoned by computers. A simple alternative to make them computer-parsable is to convert character statements to semantic statements by adopting some structured data model or standardized syntax. 
...
For the purposes of the PARAMO pipeline \citep{tarasoval2019}, only the anatomy term is necessary to guide the amalgamation of characters.
